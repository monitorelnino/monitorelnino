...
**Sozinho, todo domingo à 0h (horário de Brasília).** Um robô busca
...
- **A rotina de domingo** continua atuando no `main`; o domínio em
  branco não é tocado.
...
## Legendas, fichas e prosa: descrever, nunca interpretar

Regra única e canônica: **`docs/VOZ_EDITORIAL.md`** (consolidado 17/09/2026, antes duplicado
e parcialmente divergente deste guia). Cobre legenda, título auxiliar, subtítulo, item de
legenda, crédito, rótulo de tooltip, cartão com número, fichas e prosa das páginas de dados e
da Imprensa. Resumo: descreve o que está sendo mostrado, nunca avalia, dramatiza, atribui causa
não demonstrada ou repete a mesma ressalva metodológica mais de uma vez por página — a lista
completa dos hábitos proibidos e as exceções (Proteja-se, Para gestores) estão só lá, para não
haver duas versões da mesma regra. Portões: `scripts/verificar_legendas.js` (19) e
`scripts/verificar_voz_editorial.js`.
