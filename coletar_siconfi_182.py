...
def alvos(args) -> list:
    ...
    if "--capitais" in args:
        from coletar_sinais_risco import CAPITAL_IBGE     # fonte única dos códigos das capitais
        codigos = {str(c).zfill(7) for c in CAPITAL_IBGE.values()}
        return [t for t in todos if t[0] in codigos]
    return todos
